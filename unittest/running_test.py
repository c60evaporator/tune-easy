...
y_reg = df_reg[OBJECTIVE_VARIALBLE_REG].values
X_reg = df_reg[USE_EXPLANATORY_REG].values

# 分類用データ読込
OBJECTIVE_VARIALBLE_CLS = 'league'  # 目的変数
USE_EXPLANATORY_CLS = ['height', 'weight']  # 説明変数
df_cls = pd.read_csv(f'../sample_data/nba_nfl_2.csv')
y_cls = df_cls[OBJECTIVE_VARIALBLE_CLS].values
X_cls = df_cls[USE_EXPLANATORY_CLS].values

###### 正常系：引数なしで実行 ######

def test_svm_reg_grid_noarg():
    tuning = SVMRegressorTuning(X_reg, y_reg, USE_EXPLANATORY_REG, y_colname=OBJECTIVE_VARIALBLE_REG)
    assert tuning.grid_search_tuning()

def test_xgb_reg_grid_noarg():
    tuning = XGBRegressorTuning(X_reg, y_reg, USE_EXPLANATORY_REG, y_colname=OBJECTIVE_VARIALBLE_REG)
    assert tuning.grid_search_tuning()